...
void exs2()
{
    ...
    double failure_intensity_units_ti12 = count_rejected_units_ti2 * 1. / ((ti2 - ti1) * (count_units - count_rejected_units_ti1 + count_units - count_rejected_units_ti2) / 2);
...
void exs3()
{
    ...
    for (int i = 0; i < 30; ++i)
    {
        ...
        Qs.push_back(1 - Ps[i]);
        as.push_back(ns[i] * 1. / (count_units * sep_t));
        ns_mean.push_back((2 * count_units - ns[i] - cumulative_nds) * 1. / 2);
        lambdas.push_back(ns[i] * 1. / (sep_t * ns_mean[i]));

        cumulative_nds += nds[i];
        decreasing_nds -= nds[i];
    }

    std::cout << "N\t\t" << "ns\t\t" << "Ps\t\t" << "Qs\t\t" << "as\t\t" << "ns_mean\t\t" << "lambdas" << std::endl;
    for (int i = 0; i < 30; ++i)
        std::printf("%d\t%10d\t%10.3lf\t%10.3lf\t%10.5lf\t%10.3lf\t%10.5lf\n", i, ns[i], Ps[i], Qs[i], as[i], ns_mean[i], lambdas[i]);
}